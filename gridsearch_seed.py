...
class PrioritizedReplayBuffer:
    def __init__(self, capacity, alpha):
        ...
        self.priorities = np.zeros((capacity,), dtype=np.float32)  # 優先度
        self.position = 0
        self.alpha = alpha
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        self.priorities[self.position] = max_priority  # 新しい経験に最大の優先度を与える
...
class ContinuousSACAgent:
    def __init__(self, state_space, action_space, action_scale, alpha, gamma, tau, buffer_capacity, 
        ...
        self.alpha_values = []  # αの値を記録するリスト
        self.action_scale = torch.tensor(self.action_scale)  # Add action_scale here
    ...
    def choose_action(self, state):
        ...
        return (action * self.action_scale).cpu().detach().numpy()[0]  # Scale the action here
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            next_actions = next_actions * self.action_scale  # Scale the next actions
        ...
        new_actions = new_actions * self.action_scale  # Scale the new actions
        ...
        self.alpha_values.append(self.alpha)  # αの値を記録
        # 損失値の記録
...
np.random.seed(seed)
torch.cuda.manual_seed(seed)  # GPU用のシード
random.seed(seed)  # Python標準ライブラリの乱数シードを固定
torch.backends.cudnn.deterministic = True  # CuDNNの決定性を有効化
torch.backends.cudnn.benchmark = False  # 決定性を保つために無効化

# 環境のシードを設定
env = gym.make(task_name)
env.action_space.seed(seed)
env.observation_space.seed(seed)
env.reset(seed=seed)  # 環境のシードを固定

# マルチプロセス内でのシード設定
def set_seed_in_process(seed):
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)

# Run experiments for alpha values from 0.01 to 0.1
alpha_values = [0.01, 0.05, 0.1]

# Number of trials
num_trials = 1

# Create directory for overall trial results
...
for trial in range(1, num_trials + 1):
    ...
    for alpha in alpha_values:
        alpha = round(alpha, 4)  # αの値を小数点以下2桁に丸める
        print(alpha)
        print(f"Starting training for alpha = {alpha:.4f} | Trial {trial}")
        ...
        # target_q_network1 と target_q_network2 のパラメータが同じか確認
        #compare_network_parameters(agent.target_q_network1, agent.target_q_network2)

        # q_network1 と target_q_network2 のパラメータが同じか確認
        #compare_network_parameters(agent.q_network1, agent.target_q_network1)

        # q_network2 と target_q_network2 のパラメータが同じか確認
        #compare_network_parameters(agent.q_network2, agent.target_q_network2)
        
        # Lists for saving training and test results
        train_rewards = []
        test_rewards = []
        test = []
        ...
        # Save results for the current alpha and trial
        trial_dir = f"{overall_result_dir}/trial_{trial}"
        if not os.path.exists(trial_dir):
            os.makedirs(trial_dir)

        alpha_dir = f"{trial_dir}/alpha_{alpha:.4f}"
        if not os.path.exists(alpha_dir):
            os.makedirs(alpha_dir)

        # Save the test rewards to CSV
        save_to_csv(test, f"{alpha_dir}/Test_Return.csv", headers=["Steps", "Test_Reward[0]", "Test_Reward[1]", "Test_Reward[2]", "Test_Reward[3]", "Test_Reward[4]"])

        # Save the test rewards to CSV
        save_to_csv(test_rewards, f"{alpha_dir}/Test_Returns_Trial_{trial}.csv", headers=["Steps", "Avg_Test_Reward"])

        # Save training rewards to CSV
        train_rewards_data = list(enumerate(train_rewards, start=1))
        save_to_csv(train_rewards_data, f"{alpha_dir}/Training_Rewards_Trial_{trial}.csv", headers=["Episode", "Training_Reward"])

        # Save Q1/Q2 losses and policy losses to CSV
        #losses_data = list(zip(agent.q1_losses, agent.q2_losses, agent.policy_losses))
        #save_to_csv(losses_data, f"{alpha_dir}/Losses_Trial_{trial}.csv", headers=["Q1_Loss", "Q2_Loss", "Policy_Loss"])
        # 移動平均のウィンドウサイズを定義（例: 100）
        interval_size = 100
       ...
       # 平均結果をCSVに保存
        save_to_csv(losses_data_avg, f"{alpha_dir}/Losses_Trial_Averaged.csv", headers=["Q1_Loss_Avg", "Q2_Loss_Avg", "Policy_Loss_Avg"])

        # Save models
        torch.save(agent.q_network1.state_dict(), f"{alpha_dir}/q_network1.pth")
        torch.save(agent.q_network2.state_dict(), f"{alpha_dir}/q_network2.pth")
        torch.save(agent.policy_network.state_dict(), f"{alpha_dir}/policy_network.pth")
        ...
        plt.savefig(f"{alpha_dir}/Test_Returns_Trial_{trial}.png")
        plt.close()
        ...
        plt.savefig(f"{alpha_dir}/Training_Rewards_Trial_{trial}.png")
        plt.close()
        ...
        plt.savefig(f"{alpha_dir}/Losses_Trial_{trial}.png")
        plt.close()
        ...
        # CSVに保存
        df.to_csv(f"{alpha_dir}/all_parameter.csv", index=False)


env.close()

# 全試行後に、各アルファ値の平均テスト報酬を計算してCSVに保存する処理
alpha_averages = {}
...
with open(average_results_file, mode='w', newline='') as file:
    ...
    for alpha in alpha_values:
        total_rewards = []
        for trial in range(1, num_trials + 1):
            trial_dir = f"{overall_result_dir}/trial_{trial}/alpha_{alpha:.4f}"
        ...
        # 平均報酬をCSVに書き込み
        writer.writerow(avg_rewards)  # 最後の報酬を保存

        # コンソールにも出力
        print(f"Alpha {alpha:.2f} | Average Test Rewards: {avg_rewards}")
